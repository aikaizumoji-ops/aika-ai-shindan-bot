...
# ─── リードマグネット資料（友だち追加時に5通で配信） ─────────────
# 正本: EKAM_営業/lead_magnet/通気浸透水脈_現場診断チェックリスト.md
LEAD_MAGNET_MESSAGES = [
    {"type": "text", "text": (
        "ご登録ありがとうございます。\n\n"
        "EKAM（エカム）です。環境再生造園・環境再生土木・石場建の設計施工をしています。\n\n"
        "ご登録の御礼に、同業・専門家向け資料\n"
        "【通気浸透水脈 現場診断チェックリスト】\n"
        "をこのままお送りします（このあと4通に分けて届きます）。\n\n"
        "土の中の空気と水の流れが止まっているかどうかは、特別な機械がなくても五感と観察でほぼ判断できます。"
        "現場に着いて最初の30分で確認する順に並べています。"
    )},
    {"type": "text", "text": (
        "■ 五感でわかる「土の呼吸」停止サイン（基本10項目）\n\n"
        "3つ以上当てはまれば、通気浸透の改善を最優先で検討してください。\n\n"
        "□1 雨後2日たっても水たまりが消えない\n"
        "□2 土の表面に泥膜が張り、乾くとひび割れる\n"
        "□3 掘るとドブのような臭気（健全な土は森の匂い）\n"
        "□4 表土が冷たく硬く締まっている\n"
        "□5 ゼニゴケが一面に広がる（過湿サイン）\n"
        "□6 特定の草種だけが優占している\n"
        "□7 植栽木が下枝から枯れ上がる\n"
        ...
        "□9 落ち葉がいつまでも分解されない\n"
        "□10 雨のたびに泥水が流出する\n\n"
        "「どこが悪いか」ではなく「水と空気がどこで止まっているか」を探す目に切り替えるのが第一歩です。"
    )},
    {"type": "text", "text": (
        "■ 水の動きの読み方\n\n"
        "・滞水は道路・構造物の山側にないか（人工物が地中の水をせき止めるケースが非常に多い）\n"
        "・底から湧く清冽な水か、溜まって腐敗した水か\n"
        "・黒い酸欠泥の層がどの深さまで達しているか\n"
        "・低地の滞水と斜面の崩れ・倒木はセットで見る\n\n"
        "■ 樹木からのサイン\n\n"
        "・力枝（横に長く伸びた太枝）＝根が酸欠で横に逃げているサイン\n"
        "・空洞率だけで伐採判断しない（大木は60〜90%の空洞で長寿を全うする）\n"
        "・腐朽菌は原因ではなく結果\n"
        "・倒木の足元で、舗装が水をせき止めていないか\n"
        "・危険木の個別伐採だけで終わらせない（次の危険木を生む）"
    )},
    {"type": "text", "text": (
        "■ 浸透量の目安（ダブルリング試験）\n\n"
        "・止まった土：ほぼ浸透せず\n"
        "・裸地・泥膜の斜面：約200mm/h\n"
        "・健全な土：500mm/h以上\n"
        "→ 落ち葉がその場で分解されている＝呼吸している証拠\n\n"
        "■ 改善施工の基本順序\n\n"
        "1 崩れかけた斜面の安定が先\n"
        "2 水道（みずみち）をたぐる＝曲線で掘る\n"
        "3 点穴・水脈溝は蛇行配置\n"
        "4 杭は縦（浸透）・横（引き出し）・斜（楔）で使い分け、杭頭は地表に少し出す\n"
        "5 資材は朽ちて土に還るもの（松杭・栗石・井桁丸太・藁土嚢）\n\n"
        "■ 使わない資材\n\n"
        "×不織布フィルター（泥詰まり）\n"
        "×セメント系改良材（10〜15年で再崩壊リスク）\n"
        "×プラ貯留槽の地中埋設（液状化・浮き上がり）"
    )},
    {"type": "text", "text": (
        "■ 施工後の健全化サイン\n\n"
        "・約1週間：菌糸の白膜が出る\n"
        ...
        "・数年：水たまり消失・樹勢回復\n\n"
        "■ 歴史的実証\n\n"
        "鎌倉時代（約800年前）の盛土城跡から、松杭＋井桁丸太＋栗石＋稲わら＋落葉の構造が発掘され、"
        "いまも形を保っています。鍵は「水・空気・菌糸の共存」。特殊な材料は何も使われていません。\n\n"
        "─────────\n\n"
        "この内容を現場動画・設計判断・施工手順まで体系的に学べる「環境再生型造園オンライン講座」を準備中です。"
        "募集開始はこのLINEで最速でご案内します。\n\n"
        "EKAM｜環境再生造園・環境再生土木・石場建\n"
        "ekam.co.jp"
    )},
]
...
def handle_event(event):
    ...
    if event_type == "message" and event.get("message", {}).get("type") == "text":
        ...
        if text in ["資料", "チェックリスト", "資料希望", "診断チェックリスト"]:
            # 既存友だち向け: キーワードで現場診断チェックリストを再送
            reply(reply_token, LEAD_MAGNET_MESSAGES)

        elif text in ["無料相談", "予約", "相談"]:
            ...
            if session["step"] == 0 and session.get("mode", "") == "":
                reply(reply_token, [
                    {"type": "text", "text": "メッセージありがとうございます🌿\n\n同業・専門家向け資料【通気浸透水脈 現場診断チェックリスト】を無料でお渡ししています。\n「資料」と送っていただくと、このトークにすぐ届きます。\n\n【環境再生型造園 オンライン実践講座】も現在準備中です。募集開始はこのLINEで最速でご案内します。"},
                    {"type": "text", "text": "💡 AI活用レベル診断もできます！\n「診断スタート」と送ってみてください🤖"},
                ])

    elif event_type == "follow":
        # 友だち追加の瞬間に、御礼＋現場診断チェックリスト（5通）を自動配信
        # ※Threads CTA枠（@yoshinobu_ekam 06:30/18:00）からの導線の受け皿
        reset_session(user_id)
        reply(reply_token, LEAD_MAGNET_MESSAGES)
        print(f"[FOLLOW] lead magnet sent to user={user_id[:8]}", flush=True)
